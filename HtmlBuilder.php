...
class HtmlBuilder {
     ...
     * @return string
     */
    public function link($title, $url, $attributes = array(), $secure = null)
    {
        $url = $this->url->to($url, array(), $secure);

        if (is_null($title) || $title === false) $title = $url;
        $title = array_key_exists('escape', $attributes) ? $this->entities() : $title;
        $icon = isset($attributes['icon']) ? icon($attributes['icon']) . ' ' : '';
        return '<a href="'.$url.'"'.$this->attributes($attributes).'>'.$icon.$title.'</a>';
    }
     ...
     * @return string
     */
    public function icon($type, $attribute = null) {
        $icon = env('ICON', 'fa');
        return '<i class="'.$icon.' '.$icon.'-' . $type . ' ' . $attribute . ' "></i>';
    }
    ...
    public function label($text, $type = 'default') {
        if (empty($type) == false && strpos($type, 'label-') === false) $type = 'label-'.$type;
    ...
    public function badge($text, $type = 'default') {
        if (empty($type) == false && strpos($type, 'badge-') === false) $type = 'badge-'.$type;
        return '<span class="badge ' . $type . '">'.$text.'</span>';
    }


    /**
     * Create a bootstrap col html
     *
     * @param  integer  func_get_args()
     * @return string
     */
    function col() {
        $width = array('xs', 'sm', 'md', 'lg');
        $class = false;
        $col = func_get_args();

        if (count($col) < 4) return '';
        foreach ($width as $key => $val) {
            $class[] = 'col-'.$val.'-'.$col[$key];
        }
        if (count($col) < 8) return implode(' ', $class);
        foreach ($width as $key => $val) {
            $class[] = 'col-'.$val.'-offset-'.$col[$key+4];
        }
        return implode(' ', $class);
    }

    /**
     * Create a bootstrap push html
     *
     * @param  integer  func_get_args()
     * @return string
     */
    function push() {
        ...
        if (count($push) < 4) return '';
        foreach ($width as $key => $val) {
            $class[] = 'col-'.$val.'-push-'.$push[$key];
        }
        return implode(' ', $class);
    }

    /**
     * Create a bootstrap pull html
     *
     * @param  integer  func_get_args()
     * @return string
     */
    function pull() {
        ...
        if (count($pull) < 4) return '';
        foreach ($width as $key => $val) {
            $class[] = 'col-'.$val.'-pull-'.$pull[$key];
        }
        return implode(' ', $class);
    }
     ...
     * @return string
     */
    function status($class, $title) {
        return $this->icon('circle', $class).' '.$title;
    }
